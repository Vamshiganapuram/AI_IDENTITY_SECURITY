...
@app.get("/reports/cvss")
def cvss_table() -> FileResponse:
    return FileResponse(ROOT / "03-red-team" / "cvss_findings_table.html")


@app.get("/reports/success-matrix")
def success_matrix() -> FileResponse:
    return FileResponse(ROOT / "03-red-team" / "attack_success_matrix.html")


@app.get("/reports/compliance")
def compliance() -> FileResponse:
    return FileResponse(ROOT / "05-governance" / "compliance_mapping.html")


@app.post("/lab/chat")
def lab_chat(payload: dict[str, Any] = Body(...)) -> dict[str, Any]:
    text = str(payload.get("message") or "")
    guarded = bool(payload.get("guarded"))
    return chat(text, guarded=guarded)


@app.post("/lab/agent-b")
def lab_agent_b(payload: dict[str, Any] = Body(...)) -> dict[str, Any]:
    message = str(payload.get("message") or "")
    signature = payload.get("signature")
    guarded = bool(payload.get("guarded"))
    sig = str(signature) if signature else None
    return agent_b_privileged(message, sig, guarded=guarded)


@app.get("/lab/signed-command")
def lab_signed_command() -> dict[str, Any]:
    return signed_orchestrator_command()
